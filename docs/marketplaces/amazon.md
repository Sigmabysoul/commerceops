...
Parser version `amazon-associated-v2` never reads storage, queries Product Master,
...
classifies shipping labels and invoices, groups them only by an exact canonical
Amazon order ID, and uses the label AWB plus invoice seller SKU and explicit
quantity. It never uses adjacency or alternating page position. Exactly one
label and one invoice produce a complete association; duplicate roles or
conflicting values remain review data.
...
Amazon print cropping/output, batch compatibility changes, inventory changes,
and reporting verification are deferred beyond Batch B. The private production
PDF is not committed. The repository contains a sanitized two-page PDF plus
sanitized association and ambiguity regression cases.
