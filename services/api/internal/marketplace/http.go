...
func (h *HTTPHandler) Jobs(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseMultipartForm(MaxUploadBytes); err != nil {
		httpserver.WriteError(w, 400, "INVALID_UPLOAD", "Upload must be a PDF smaller than 20 MiB")
		return
	}
	file, header, err := r.FormFile("file")
	if err != nil {
		httpserver.WriteError(w, 400, "INVALID_UPLOAD", "PDF file is required")
		return
	}
	defer file.Close()
	...
	result, err := h.service.Upload(r.Context(), p, header.Filename, data)
	if writeError(w, err) {
		return
	}
