...
func TestMeeshoUsesCentralEcommerceOutboundEvent(t *testing.T) {
	f := setup(t)
	ctx := context.Background()
	...
	items, replay, err := f.service.ConfirmEcommerceOutbound(ctx, f.principal, batch, OutboundInput{IdempotencyKey: "meesho-dispatch"})
	if err != nil || replay || len(items) != 1 || items[0].QuantityDelta != -5 || items[0].ResultingBalance != 3 || items[0].ReferenceID == nil || *items[0].ReferenceID != batch {
		t.Fatalf("outbound=%#v replay=%v err=%v", items, replay, err)
	}
	items, replay, err = f.service.ConfirmEcommerceOutbound(ctx, f.principal, batch, OutboundInput{IdempotencyKey: "meesho-dispatch"})
	if err != nil || !replay || len(items) != 1 {
	...
	scan(t, f.db, `SELECT count(*) FROM audit_logs WHERE company_id=$1 AND action='inventory.ecommerce_out' AND target_id=$2`, []any{f.company, batch}, &audits)
	if events != 1 || transactions != 1 || audits != 1 {
		t.Fatalf("trace events=%d transactions=%d audits=%d", events, transactions, audits)
	}
}
