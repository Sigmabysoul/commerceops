...
func TestSellerAccountsArePermissionAndCompanyScoped(t *testing.T) {
	...
	if account.IdentityName != identity.DisplayName {
		t.Fatalf("identity name = %q, want %q", account.IdentityName, identity.DisplayName)
	}
	if _, err = service.Accounts(ctx, auth.Principal{UserID: userID, CompanyID: companyB}, ""); !errors.Is(err, authorization.ErrPermissionDenied) {
		t.Fatalf("other company access result: %v", err)
	}
	if _, err = service.SaveAccount(ctx, principalA, "", AccountInput{MarketplaceKey: "flipkart", BusinessIdentityID: identity.ID, InternalKey: account.InternalKey, DisplayName: "Duplicate", IntegrationMode: "manual_upload", Status: "active"}); !errors.Is(err, ErrConflict) {
		t.Fatalf("duplicate account key result: %v", err)
	}
}

func mustAccountScan(t *testing.T, db *pgxpool.Pool, query string, args []any, destination ...any) {
	t.Helper()
	if err := db.QueryRow(context.Background(), query, args...).Scan(destination...); err != nil {
		t.Fatalf("fixture query: %v", err)
	}
}
func mustAccountExec(t *testing.T, db *pgxpool.Pool, query string, args ...any) {
	t.Helper()
