...
export function BatchPrinting() {
  ...
  return <section className="batch-printing">
    <div className="product-heading"><div><p className="eyebrow">Shared marketplace workflow</p><h2>Batch printing</h2><p className="muted">Select processed marketplace orders, review Product Master totals, and generate traceable output.</p></div><div className="workspace-controls"><select aria-label="Batch marketplace" value={marketplace} onChange={(event) => selectMarketplace(event.target.value as MarketplaceKey)}><option value="flipkart">Flipkart</option><option value="amazon">Amazon</option><option value="meesho">Meesho</option><option value="myntra">Myntra</option><option value="snapdeal">Snapdeal</option></select><button className="secondary" disabled={operation !== null} onClick={() => loadEligible().catch((cause) => setError(message(cause)))}>Refresh orders</button></div></div>
