...
import { QualityAnalytics } from "@/components/quality-analytics";
...
import { TraceabilityWorkspace } from "@/components/traceability-workspace";
...
export function CorePlatform() {
  ...
  async function login(event: FormEvent<HTMLFormElement>) {
    ...
    try { await coreAPI.login(String(data.get("email")), String(data.get("password"))); await load(); }
  ...
  return <main className="shell"><PWARegistration /><header><div><p className="eyebrow">CommerceOps · Core Platform</p><h1>{company?.name ?? "Company administration"}</h1><p>{user.email}</p></div><button onClick={() => coreAPI.logout().then(() => setUser(null))}>Log out</button></header>
    ...
    </div><QuickPrint /><AutomationWorkspace /><TraceabilityWorkspace /><OperationsDashboard /><QualityAnalytics /><ConsignmentWorkspace employees={employees} /><ReturnsWorkspace /><ProductMaster /><FlipkartProcessing /><MeeshoProcessing /><MyntraProcessing /><SnapdealProcessing /><BatchPrinting /><InventoryWorkspace /></main>;
}

function Login({ onSubmit, error }: { onSubmit: (event: FormEvent<HTMLFormElement>) => void; error: string }) {
  return <main className="login"><section><p className="eyebrow">CommerceOps</p><h1>Sign in</h1><p>Use your CommerceOps account.</p><form onSubmit={onSubmit}><label>Email<input name="email" type="email" autoComplete="username" required /></label><label>Password<input name="password" type="password" autoComplete="current-password" required /></label><button type="submit">Sign in</button></form>{error && <p className="error" role="alert">{error}</p>}</section></main>;
}
