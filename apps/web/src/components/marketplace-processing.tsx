...
export function MarketplaceProcessing({ api, marketplace, phase }: Props) {
  ...
  return <section className="flipkart"><div className="product-heading"><div><p className="eyebrow">{phase}</p><h2>{displayName} processing</h2><p className="muted">Upload a {displayName} PDF. Processing continues in the shared bounded background queue.</p></div></div>
    <section className="panel"><form className="upload-row" onSubmit={upload}><label>{displayName} PDF<input name="file" type="file" accept="application/pdf,.pdf" required /></label><button disabled={uploading}>{uploading ? "Uploading…" : "Upload and process"}</button></form>{error && <p className="error" role="alert">{error}</p>}{duplicate && <p className="notice">This exact source file was already uploaded. Showing its existing job.</p>}</section>
    {details && <section className="panel results"><div className="status-line"><h2>Processing results</h2><span className={`status status-${details.job.status}`}>{details.job.status.replace("_", " ")}</span></div><p className="muted">{details.job.processed_pages}/{details.job.total_pages} pages · parser {details.job.parser_version}</p>{["needs_review", "failed", "processed"].includes(details.job.status) && <button onClick={retry}>Reprocess with current SKU training</button>}
