...
export function ReturnsWorkspace() {
  ...
  return <section className="returns-workspace">
    <div className="product-heading"><div><p className="eyebrow">Phase 8</p><h2>Returns and cancellations</h2><p className="muted">Physical evidence, explicit disposition, and traceable inventory impact.</p></div><div className="workspace-controls"><select aria-label="Marketplace" value={marketplace} onChange={(event) => setMarketplace(event.target.value)}><option value="">All marketplaces</option><option value="flipkart">Flipkart</option><option value="amazon">Amazon</option><option value="meesho">Meesho</option><option value="myntra">Myntra</option></select><button className="secondary" disabled={busy} onClick={load}>{busy ? "Loading…" : "Refresh"}</button></div></div>
