...
export type DashboardReport = {
  ...
  inventory?: { current_on_hand: number; current_reserved: number; current_available: number; stock_in: number; stock_out: number; adjustments: number; net_movement: number };
  product_movements: { product_id: string; internal_code: string; product_name: string; order_quantity: number; stock_in: number; stock_out: number; adjustments: number; net_movement: number }[];
